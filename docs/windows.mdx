...
> This section applies to the [TurboQuant fork](https://github.com/amarce/ollama) only.

To install the TurboQuant version (which adds automatic KV cache compression on NVIDIA GPUs):

1. Download `OllamaSetup-turboquant.exe` from [Releases](https://github.com/amarce/ollama/releases/tag/v2.0.2-turboquant)
...
The installer can upgrade an existing official Ollama installation in-place (same install directory).
...
TurboQuant auto-enables on NVIDIA CUDA GPUs with Flash Attention support. Control it via the desktop app Settings (Auto/On/Off dropdown) or the `OLLAMA_TURBOQUANT` environment variable.
